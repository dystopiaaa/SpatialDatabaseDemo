...
namespace InformationManageSystem
{
    public partial class StatForm : Form
    {
        ...
        private string wtFrom = null, wtTo = null;
        public string WtFrom
        {
            set { wtFrom = value; }
        }
        public string WtTo
        ...
        public StatForm(ref SqlConnection con)
        {
            InitializeComponent();
            this.con = con;
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case "学历":
                        sql = "select education as 学历,count(edu_id) as 人数 " +
                            "from employee_education group by education";
                        break;
                    case "婚姻状况":
                        sql = "select married as 婚姻状况,count(em_id) as 人数 " +
                            "from employee_marry group by married";
                        break;
                    case "岗位":
                        sql = "select job_name as 岗位,count(id) as 人数 " +
                            "from employee,sys_job where employee.job_id=sys_job.job_id " +
                            "group by job_name";
                        break;
                    case "参加工作时间":                       
                        WorkTime wt = new WorkTime(this);
                        wt.ShowDialog();
                        if (wtFrom != null && wtTo != null) {
                            sql = "select min(work_time) as 从,max(work_time) as 到,count(id) as 人数 " +
                            "from employee where work_time between '" + wtFrom + "' and '" + wtTo + "'";                          
                        }
                        else { MessageBox.Show("参加工作时间范围输入错误！"); }
                        break;
                }
        ...
        {
            string fileName = this.Name;//可以在这里设置默认文件名
